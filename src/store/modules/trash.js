...
const getters = {
  ...
  curTrashNote:(state,getters)=>{
    if(!state.curTrashNotesId) return getters.trashNotes[0] || {}
    return state.trashNotes.find(note => note.id == state.curTrashNotesId) || {}
  },
...
const mutations = {
  ...
  setCurTrashNote(state,payload){
    state.curTrashNote = payload.curTrashNoteId
  }
}
